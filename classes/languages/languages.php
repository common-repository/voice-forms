...
class VfLanguage
{
    ...
    const WIDGET_MESSAGES = 'vfWidgetMessages';
    const PLACEHOLDER = 'placeholder';

    const ERRORS = 'vfErrorLibrary';
     ...
     * 
     * @return Array $vf_all_lang_set  Array of language arrays
     */
    public static function get_all_languages()
    {
        $vf_all_lang_set = array();

        try {

            if (!empty(self::$gcp_language_set)) {
                $vf_all_lang_set = self::$gcp_language_set;
            } else {
                $vf_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
            }

            uksort($vf_all_lang_set, 'strcasecmp');
        } catch (\Exception $err) {
            $vf_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
        }

        return $vf_all_lang_set;
    }
    ...
    public static function get_short_phrase_lang_code($lang_code)
    {
        ...
        $vf_valid_lang_code = false;

        if (self::gcp_supported($lang_code)) {
            $vf_lang = self::$gcp_language_set[$lang_code];
            $vf_valid_lang_code = true;
        }

        if ($vf_valid_lang_code === true && array_key_exists(self::SHORT_PHRASE_LANG, $vf_lang)) {
            $short_phrase_lang = $vf_lang[self::SHORT_PHRASE_LANG];
        }
